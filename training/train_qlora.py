"""
LoRA fine-tuning of Qwen3.6-35B-A3B for pharma MMM domain adaptation.
...
def format_chat(example: dict) -> dict:
    """Format messages into chat template string for tokenizer."""
    return {"messages": example["messages"]}


def main():
    parser = argparse.ArgumentParser(description="QLoRA fine-tune Qwen3.6-35B-A3B")
    ...
    parser.add_argument("--epochs", type=int, default=15,
                        help="Number of training epochs (15+ recommended for small datasets)")
    ...
    parser.add_argument("--lora-r", type=int, default=16,
                        help="LoRA rank (16 for small datasets to avoid overfitting)")
    ...
    print("=" * 60)
    print("QLoRA Fine-Tuning: Qwen3.6-35B-A3B → Pharma MMM Agent")
    print("=" * 60)
    ...
    # B200 GPUs on fat partition have 192GB VRAM each.
    ...
    # --- 4. LoRA config ---
    # For MoE with small datasets (<50 examples), only target shared attention layers.
    # Expert MLP layers (gate_proj, up_proj, down_proj) are sparsely activated —
    # adapting them with few examples causes overfitting to specific expert routing
    # patterns. Attention layers are shared across all tokens and generalize better.
    lora_config = LoraConfig(
        ...
        target_modules=[
            "q_proj", "k_proj", "v_proj", "o_proj",  # shared attention only
        ],
    )
